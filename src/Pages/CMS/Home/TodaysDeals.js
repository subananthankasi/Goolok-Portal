...
const TodaysDeals = () => {
  ...
  const handlePreview = (row) => {
    setPreview(true);
    const url = `${IMG_PATH}/cms/todaydeals/${row.image}`;
    setPreviwUrl(url);
  };
  const columns = [
    {
      ...
      cell: (row) =>
        row.image ? (
          <div onClick={() => handlePreview(row)} style={{ cursor: "pointer" }}>
            <img
              src={`${IMG_PATH}/cms/todaydeals/${row.image}`}
              alt={row.title}
              style={{
                width: "100px",
                height: "80px",
  ...
  const handleEdit = (row) => {
    ...
    setPreviewImage(`${IMG_PATH}/cms/todaydeals/${row.image}`);
    formik.setFieldValue("image", row.image || "");
  ...
  const fetchRoles = async () => {
    try {
      const response = await axios.get(`${API_BASE_URL}/todaydealsviewall`);
  ...
  const onSubmit = async (values) => {
    ...
    try {
      const response = await axios.post(`${API_BASE_URL}/todaydeals`, values, {
  ...
  const handleconfirmopendelete = async () => {
    try {
      await axios.delete(`${API_BASE_URL}/todaydeals/${selectedRowId}`);
      fetchRoles();
  ...
  return (
    <>
          ...
          <div className="card">
            <div className="card-header">
              <div className="d-flex justify-content-between">
                <h4 className="page_heading">Todays Deals Reports</h4>
      ...
      >
        <Modal.Header>
          <Modal.Title>TodaysDeals </Modal.Title>
        </Modal.Header>
