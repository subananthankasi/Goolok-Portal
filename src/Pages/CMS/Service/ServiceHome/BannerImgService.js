...
const BannerImgService = () => {
  ...
  const handlePreview = (row) => {
    setPreview(true)
    const url = `${IMG_PATH}/cms_service/banners/${row.image}`
    setPreviwUrl(url)
  }
      ...
      cell: (row) =>
        row.image ? (
          <div onClick={() => handlePreview(row)} style={{ cursor: "pointer" }}>
            <img
              src={`${IMG_PATH}/cms_service/banners/${row.image}`}
              alt={row.title}
              style={{
                width: "100px",
                height: "80px",
  ...
  const handleEdit = (row) => {
    ...
    setPreviewImage(`${IMG_PATH}/cms_service/banners/${row.image}`);
    formik.setFieldValue("image", row.image || "");
  ...
  const fetchRoles = async () => {
    try {
      const response = await axios.get(`${API_BASE_URL}/bannerimagesviewall`, {
      });
  ...
  const handleconfirmopendelete = async () => {
    try {
      await axios.delete(`${API_BASE_URL}/bannerimages/${selectedRowId}`);
      fetchRoles();
  ...
  return (
    <>
            ...
            <div className="card-header">
              <div className="d-flex justify-content-between">
                <h4 className="page_heading">
                  Service Banner Image View Table
                </h4>
                <button
                  type="button"
                  className="btn1"
      ...
      >
        <Modal.Header>
          <Modal.Title> Service Banner Image </Modal.Title>
        </Modal.Header>
            ...
            <div className="col-md-8 d-flex gap-5 mb-3">
              ...
              <div className="form-check">
                <input
                  ...
                  id="webOption"
                  value="web"
                  checked={formik.values.title === "web"}
              ...
              <div className="form-check">
                <input
                  ...
                  id="appOption"
                  value="app"
                  checked={formik.values.title === "app"}
                  onChange={formik.handleChange}
                />
                <label
                  className="form-check-label text-capitalize"
...
export default BannerImgService;
