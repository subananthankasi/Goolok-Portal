...
const PaymentForLegalTele = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(
        `${API_BASE_URL}/legalpayment?id=${staffid.loginid}&status=pending`,
        {
          headers: {
  ...
  const handleRowSelect = (args) => {
    ...
    navigate(
      `/telecalling_paymentlegal/${encryptData(rowData.enqid)}/${encryptData(
        rowData.id
      )}/${encryptData("pending")}`
    );
  };
            ...
            {loading ? (
              ...
              <div className="col-12">
                <div className="card-body p-1">
                  <h4 className="page_heading"> Proposal Report</h4>
...
export default PaymentForLegalTele;
