...
import propertyID from "../../../common/PropertyID";
import { TabView, TabPanel } from "primereact/tabview";
import { Badge } from "primereact/badge";

function TelecallingDocVerification() {
  ...
  const fetch = async () => {
    try {
      ...
      if (staffid.Login === "admin") {
        response = await axios.put(`${API_BASE_URL}/telecalldocument/all`);
      } else {
      ...
      const formattedData = response.data?.map((data, index) => {
        const days =
          Math.floor((new Date() - new Date(data.created_at)) / 86400000) || 0;
      ...
      const sellData = formattedData?.filter(item => item.type === "sale");
      const serviceData = formattedData?.filter(item => item.type === "service");
  ...
  const handleRowSelect = (args) => {
    ...
    navigate(`/telecalling_fileupload/${rowData.id}/${rowData.userid}/pending`);
  };
                    ...
                    <div>
                      <TabView>
                        <TabPanel
                          header={
                            <span>
                              Sell <Badge value={sellData ? sellData?.length : 0} severity="info" />
                            </span>
                          }
                        >
                        ...
                        <TabPanel
                          header={
                            <span>
                              Service <Badge value={serviceData ? serviceData?.length : 0} severity="info" />
                            </span>
                          }
                        >
                          <div className="col-lg-12 mb-4 mt-4">
...
export default TelecallingDocVerification;
