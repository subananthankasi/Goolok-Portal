...
const PriceProposalTele = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(
        `${API_BASE_URL}/telecallproposal?id=${staffid.loginid}&status=pending`,
        {
          headers: {
  ...
  const handleRowSelect = (args) => {
    ...
    navigate(
      `/telecalling_proposal/${encryptData(rowData.enqid)}/${encryptData(
        rowData.id
      )}/${encryptData("pending")}`
    );
  };
...
export default PriceProposalTele;
