...
const WholeAfterSale = ({
  ...
  const handleUploadEdit = (row) => {
    setNewUpload(true);
    formik1.setFieldValue("id", row.id);
    formik1.setFieldValue("patta", row.patta_doc || "");
    formik1.setFieldValue("file", row.ec_doc || "");
    formik1.setFieldValue("sale_deed_softcopy", row.sd_soft_copy || "");
    formik1.setFieldValue("patta_application", row.patta_app || "");
    formik1.setFieldValue("sale_deed_softcopy_old", row.sd_soft_copy || "");
    formik1.setFieldValue("patta_application_old", row.patta_app || "");
    formik1.setFieldValue("patta_old", row.patta_doc || "");
    formik1.setFieldValue("file_old", row.ec_doc || "");
    setFilePreviews({
  ...
  const fileValidation = yup.object().shape({
    // sale_deed_softcopy: yup.string().required("sale_deed_softcopy is required"),
    // file: yup.string().required("ec is required"),
    // patta: yup.string().required("patta is required"),
    // patta_application: yup.string().required("patta_application is required"),
    // sale_deed_softcopy: yup
    //   .mixed()
    //   .test(
    //     "sale_deed_softcopy",
    ...
    //     function (value) {
    //       const { sale_deed_softcopy_old } = this.parent;
    //       if (sale_deed_softcopy_old) return true;
    //       return value instanceof File;
    //     }
    //   ),
    // file: yup.mixed().test("file", "EC file is required", function (value) {
  ...
  const fileOnSubmit = async (values) => {
    setFileLoading(true);
    try {
      const payload = {
        ...values,
      ...
      await axios.post(`${API_BASE_URL}/aftersaledpt`, payload, {
        headers: {
          "Content-Type": "multipart/form-data",
        },
      });
        ...
        <form onSubmit={formik.handleSubmit} autoComplete="off">
          <div className="row">
            ...
            <div className="col-4">
              ...
              <div className="form-group mt-2">
                <label className="form-label">
                  Sale deed hard Copy :<span style={{ color: "red" }}>*</span>
                </label>
                <select
                  type="text"
                  className="form-select"
      ...
      >
        <form onSubmit={formik1.handleSubmit} autoComplete="off">
          ...
          {fileType === "Sale Deed Soft" && (
            <div className="form-group mt-2">
              ...
              <input
                ...
                onBlur={formik1.handleBlur}
              />
              {formik1.errors.sale_deed_softcopy &&
                formik1.touched.sale_deed_softcopy ? (
                <p style={{ color: "red", fontSize: "12px" }}>
                  {formik1.errors.sale_deed_softcopy}
                </p>
              ) : null}
          ...
          {fileType === "Patta" && (
            <div className="form-group mt-2">
              <label className="form-label">
                Patta :<span style={{ color: "red" }}>*</span>
              </label>
              <input
                type="file"
                className="form-control"
                ...
                id="patta"
                onChange={(event) => {
                  formik1.setFieldValue("patta", event.currentTarget.files[0]);
                }}
                onBlur={formik1.handleBlur}
              />
              {formik1.errors.patta && formik1.touched.patta ? (
                <p style={{ color: "red", fontSize: "12px" }}>
                  {formik1.errors.patta}
                </p>
              ) : null}
            </div>
          )}
          {fileType === "Patta Application" && (
            <div className="form-group mt-2">
              <label className="form-label">
                Patta Application :<span style={{ color: "red" }}>*</span>
              </label>
              <input
                ...
                onBlur={formik1.handleBlur}
              />
              {formik1.errors.patta_application &&
                formik1.touched.patta_application ? (
                <p style={{ color: "red", fontSize: "12px" }}>
                  {formik1.errors.patta_application}
                </p>
              ) : null}
          ...
          {filePreviews.patta_application &&
            fileType === "Patta Application" && (
              <button
                type="button"
                className="btn btn-sm btn-link ms-2"
                onClick={() => openFilePreview(filePreviews.patta_application)}
                title="View file"
              >
                <VisibilityIcon fontSize="small" />
              </button>
            )}
          {filePreviews.sale_deed_softcopy && fileType === "Sale Deed Soft" && (
            <button
              type="button"
              className="btn btn-sm btn-link ms-2"
              onClick={() => openFilePreview(filePreviews.sale_deed_softcopy)}
              title="View file"
            >
              <VisibilityIcon fontSize="small" />
            </button>
          )}
          {filePreviews.file && fileType === "EC" && (
            <button
              type="button"
              className="btn btn-sm btn-link ms-2"
              onClick={() => openFilePreview(filePreviews.file)}
          ...
          {filePreviews.patta && fileType === "Patta" && (
            <button
              type="button"
              className="btn btn-sm btn-link ms-2"
              onClick={() => openFilePreview(filePreviews.patta)}
