...
import { AddMoreLawyerDocumentDetails } from "./AddMoreLawyerDocumentDetails";
import ConfirmationModal from "../../../../Utils/ConfirmationModal";
import DeleteIcon from "@mui/icons-material/Delete";
import Button from '@mui/material/Button';
import { Dialog } from 'primereact/dialog';
import { RedoModel } from "../../../Enquiry/Reusable/RedoStatus";
import { AddMoreView } from "../../../Enquiry/Reusable/AddMoreView";
...
export const LawyerAddDocument = (props) => {
  ...
  const columns = [
      ...
      cell: (row) => (
        <>
          {row.upload_type == "lawyer" ? (
            ...
            >
              {((props.data.status === "pending" || props.data.status === "complete") && staffid.Login == "staff") && props.data.pagetype !== "reminder" && enquiryDoumentData?.status !== "booking" ? 'Fill Details..' : 'View'}
            </button>
          ) : (
            <button
              type="button"
    ...
    ...(staffid.Login == "staff" && (props.data.status == "pending" || props.data.status == "complete") && props.data.pagetype !== "reminder" && enquiryDoumentData?.status !== "booking"
      ? [
        {
          name: "Actions",
          cell: (row) =>
            row.upload_type === "lawyer" ? (
              <button type="button" className="btn btn-outline-danger delete" onClick={() => openDelete(row.id)}>
                <DeleteIcon />
              </button>
            ) : null,
        },
          ...
          <div className="modal-content">
            <div className="d-flex" style={{ alignItems: "center" }}>
              ...
              <button
                ...
                onClick={() => { closeModal(); setSelectedData(null) }}
              >
            ...
            <div className="card-body p-3">
              <form>
                ...
                <div className="text-end mt-4">
                  <button className="btn1 me-1" type="button" onClick={() => setSelectedData(null)}>
                    Clear
                  </button>
                  <button className="btn1" onClick={handleSubmit} disabled={loading} >
                    {loading ? (
                      <>
...
const DocumentUpload = ({ isOpen, closeModal, uploadDocData, fetchEnqData }) => {
