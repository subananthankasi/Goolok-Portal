...
function Pending() {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(`${API_BASE_URL}/mandatory?id=${staffid.loginid}&status=pending`, {
        headers: {
          "Pr-Root": "land"
        }
      });
  ...
  const handleRowSelect = (args) => {
    ...
    navigate(`/mandatory_docs_update/${encryptData(rowData.enqid)}/${encryptData(rowData.id)}/${encryptData('pending')}/${encryptData(rowData.subpro_name)}`);
  };
