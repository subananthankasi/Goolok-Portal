...
const CompleteMarketCom = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(
        `${API_BASE_URL}/market?id=${staffid.loginid}&status=complete`,
        {
          headers: {
            "Pr-Root": "commercial",
          },
  ...
  const handleRowSelect = (args) => {
    ...
    navigate(
      `/commercial_marketresearch/${encryptData(rowData.enqid)}/${encryptData(
        rowData.id
      )}/${encryptData("complete")}/${encryptData(rowData.subpro_name)}`
    );
  };
  return (
    <>
...
export default CompleteMarketCom;
