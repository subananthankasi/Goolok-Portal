...
const Bank = () => {
    ...
    const data = useSelector((state) => state.bankData?.get?.data);

    const isLoading = useSelector(
        (state) => state.bankData?.get?.loading
    );
    const postLoading = useSelector(
        (state) => state.bankData?.post?.loading
    );
    const updateLoading = useSelector(
        (state) => state.bankData?.update?.loading
    );
    ...
    const handleDelete = () => {
        dispatch(bankDeleteThunk(deleteId)).then(() => {
            dispatch(bankGetThunk());
    ...
    const onSubmit = async (values) => {
        const newData = {
            ...values,
            bank_name: cleanText(values.bank_name),
        };
        if (editing) {
            const res = await dispatch(bankUpdateThunk(newData));

            if (bankUpdateThunk.fulfilled.match(res)) {
                formik.resetForm();
                setEditDialog(false);
                dispatch(bankGetThunk());
            ...
            if (bankUpdateThunk.rejected.match(res)) {
                formik.setFieldError("bank_name", res?.payload?.messages?.bank_name);
            }
        } else {
            const res = await dispatch(bankPostThunk(newData));
            if (bankPostThunk.fulfilled.match(res)) {
                formik.resetForm();
                dispatch(bankGetThunk());
                Toast({ message: "Successfully Submited", type: "success" });
            }

            if (bankPostThunk.rejected.match(res)) {
                formik.setFieldError("bank_name", res?.payload?.messages?.bank_name);
            }
        }
    };
    ...
    const searchColumns = ["sno", "bank_name", "status"];
                        ...
                        <div className="col-lg-4 col-sm-6">
                            <div className="card">
                                <div className="card-header">
                                    <h4 className="page_heading">Add Banks </h4>
                                </div>
