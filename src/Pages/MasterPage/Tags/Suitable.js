...
const Suitable = () => {
  ...
  const onSubmit = async (values) => {
    const newData = {
      ...values,
      suitabletag: cleanText(values?.suitabletag),
    };
    if (editing) {
      const res = await dispatch(suitableUpdateThunk(newData));

      if (suitableUpdateThunk.fulfilled.match(res)) {
        formik.resetForm();
        setEditDialog(false);
      ...
      const res = await dispatch(suitablePostThunk(newData));
  ...
  const handleEdit = (row) => {
    setEditDialog(true);
    formik.setFieldValue("suitabletag", row.suitable_tag);
  ...
  const formik = useFormik({
    ...
    validationSchema: yup.object().shape({
      suitabletag: yup.string().required("suitable is required !!"),
    }),
    onSubmit,
  });
  ...
  const searchColumns = ["sno", "suitable_tag", "status"];
          ...
          <div className="row">
            <div className="mb-3 col-md-12">
              ...
              <input
                ...
                onBlur={formik.handleBlur}
              />
              {formik.errors.suitabletag && formik.touched.suitabletag ? (
                <p style={{ color: "red", fontSize: "14px" }}>
                  {formik.errors.suitabletag}
                </p>
              ) : null}
