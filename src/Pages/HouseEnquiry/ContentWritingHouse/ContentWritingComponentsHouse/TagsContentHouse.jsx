...
const TagsContentHouse = ({ eid, id, status }) => {
    ...
    const suitableOption =
        suitableTag?.map((item) => ({
            name: item.suitable_tag,
            code: item.id,
        })) || [];
    const propertyOption =
        propertytag?.map((item) => ({
    ...
    const suitGetData = useSelector(
        (state) => state.tagData?.get?.data?.[0]?.suitable_tags
    );
    const propGetData = useSelector(
        (state) => state.tagData?.get?.data?.[0]?.property_tags
    );
    ...
    useEffect(() => {
        const updateDataAsync = async () => {
            if (triggerChild) {
                try {
                    await axios
                        .post(`${API_BASE_URL}/contentdpt/createtags`, updateData, {
                            headers: {
                    ...
                    <form onSubmit={formik.handleSubmit} autoComplete="off">
                        <div className="container">
                            <div className="form-group">
                                <label htmlFor="suitableTags" className="form-label">
                                    Suitable Tags :{" "}
                                </label>
                                <MultiSelect
                                    value={suitableOption.filter((option) =>
                                        selectedSuId.includes(option.code)
                                    )}
                                    name="suitableTags"
                            ...
                            <div className="mt-2 col-md-12">
                                ...
                                <MultiSelect
                                    value={propertyOption.filter((option) =>
                                        selectedPropId.includes(option.code)
                                    )}
                                    name="propertyTags"
...
export default TagsContentHouse;
