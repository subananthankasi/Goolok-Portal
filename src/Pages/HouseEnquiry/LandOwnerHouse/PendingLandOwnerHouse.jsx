...
const PendingLandOwnerHouse = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(`${API_BASE_URL}/enquirylandowner?id=${staffid.loginid}&status=pending`, {
        headers: {
          "Pr-Root": "house"
        }
      });
  ...
  const handleRowSelect = (args) => {
    ...
    navigate(`/house_landowneragreement/${encryptData(rowData.enqid)}/${encryptData(rowData.id)}/${encryptData('pending')}/${encryptData(rowData.subpro_name)}`);
  };
...
export default PendingLandOwnerHouse;
