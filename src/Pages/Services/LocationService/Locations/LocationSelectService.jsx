...
const LocationSelectService = ({ eid, id, status, pagetype }) => {
    ...
    const handleConfirm = async () => {
        ...
        try {
            await axios.get(`${API_BASE_URL}/locationservice/${eid}/${id}`);
            Toast({ message: "Successfully Updated", type: "success" });
            setConfirmLoading(false)
            navigate("/location_service#Complete")
        } catch (error) {
    ...
    return isLoaded ? (
        <>
            ...
            <div className="col-12 mt-4">
                <div className="card shadow border-0">
                    <div className="card shadow border-0 p-4">
                        <h6>Google map location mapping</h6>
                        <hr />
                        <table className="table table-hover table-bordered " >
                            <thead>
                                <tr>
                                    <th className="text-center table-bg" style={{  fontWeight: "400" }}> S.no</th>
                                    <th className="text-center table-bg" style={{  fontWeight: "400" }}>Survey No </th>
                                    <th className="text-center table-bg" style={{  fontWeight: "400" }}> Lat & Lng </th>
                                    {staffid.Login === "staff" && (status === "pending" || status === "complete") && pagetype !== "reminder" && enquiryDoumentData?.status !== "live" ? (
                                        <th className="text-center table-bg" style={{  fontWeight: "400" }}> Actions </th>
                                            ...
                                            {staffid.Login === "staff" && (status === "pending" || status === "complete") && pagetype !== "reminder" && enquiryDoumentData?.status !== "live" ? (
                                                <td className="text-center">
                                                    {item.location ? (
                                                        ...
                                                        <button className="btn1 me-2" onClick={() => openDialog(item.id)}>
                                                            Add
                                                        </button>
                                                    )}
                                                </td>
                        ...
                        {staffid.Login === "staff" && (status === "pending") && pagetype !== "reminder" && (
            ...
            <Dialog visible={newDialog} style={{ width: '62rem' }} breakpoints={{ '960px': '75vw', '641px': '90vw' }} header="Add Location" modal className="p-fluid" onHide={() => { setNewDialog(false); formik.resetForm() }} >
                    ...
                    <div>
                        <div className="form-group mt-3">
                            <div className="row mb-3 align-items-center">
                                ...
                                {(status === "pending" || status === "complete") && pagetype !== "reminder" && staffid.logintype === "staff" ? (
                                    <div className="col-1">
                        ...
                        <div className="mt-4">
                            <div className="" style={{ width: "60%", margin: "auto" }}>
                                <div className="card mt-2" style={{ position: "absolute", zIndex: "1", width: "50%" }}>
            ...
            <Dialog visible={editDialog} style={{ width: '62rem' }} breakpoints={{ '960px': '75vw', '641px': '90vw' }} header="Edit Location" modal className="p-fluid" onHide={() => { setEditDialog(false); formik.resetForm() }} >
                <form onSubmit={formik.handleSubmit} autoComplete="off">
                    <div>
                        <div className="form-group mt-3">
                            <div className="row mb-3 align-items-center">
...
export default LocationSelectService