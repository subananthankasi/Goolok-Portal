...
function PendingLawyerLegal() {
    ...
    //     try {
    //         const response = await axios.get(`${API_BASE_URL}/lawyer?id=${staffid.loginid}&status=pending`, {
    //             headers: {
    //                 "Pr-Root": "land",
    //                 "Level": "service",
    //             }
...
const fetchData = useCallback(async () => {
    try {
        const response = await axios.get(
            `${API_BASE_URL}/lawyer?id=${staffid.loginid}&status=pending`,
            {
                headers: {
                    "Pr-Root": "land",
                    "Level": "service",
...
useEffect(() => {
    ...
    const handleRowSelect = (args) => {
        ...
        navigate(`/legal_opinion_lawyer_update/${encryptData(rowData.enqid)}/${encryptData(rowData.id)}/${encryptData('pending')}`);
    };
    ...
    const ProjectID = (props) => {
        return (
            <div className="btn btn_pdf light btn-warning text-dark">
                {props.propertyid}
            </div>
        );
    };
                                        ...
                                        <div className="col-lg-12 mb-4 mt-4">
                                            ...
                                            >
                                                <ColumnsDirective>
                                                    ...
                                                    {staffid.Login === "admin" && (
                                                        <ColumnDirective
                                                            headerText="Assign Ticket"
                                                            width="150"
                                                            template={(props) => props.taken_by}
                                                        />
                                                    )}
                                                    <ColumnDirective
                                                        field="customer"
...
export default PendingLawyerLegal;
