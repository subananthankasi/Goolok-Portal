...
import TalukDetails from "./TalukDetails";
...
const PattaComponent = ({ eid, id, status, pagetype }) => {
      ...
      cell: (row) => (
        <>
          <div className="d-flex">
            <button className='btn' onClick={() => viewFile(row)} ><PictureAsPdfIcon sx={{ fontStyle: "45px" }} />  </button>
          </div>
        </>
      ),
      sortable: true,
  ...
  const handleDelete = async (row) => {
    try {
      const response = await axios.delete(`${API_BASE_URL}/pattaApp/${deleteId}`)
      fetch()
  ...
  const handleConfirm = async () => {
    setPostLoading(true)
    try {
      await axios.get(`${API_BASE_URL}/pattaApp/${eid}/${id}`);
      Toast({ message: "Successfully Updated", type: "success" });
      setPostLoading(false)
      navigate("/patta_appalication#Complete")
    } catch (error) {
        ...
        <div className="card shadow border-0">
          <div className="card shadow border-0 p-4">
            <div className="d-flex justify-content-between">
              <h6>Patta Application</h6>
              {staffid.logintype == "staff" && (status === "complete" || status === "pending") && pagetype !== "reminder" && enquiryDoumentData?.status !== "live" && (
                <button className="btn1" onClick={() => setVisible(true)}>Add patta Document</button>
          ...
          <div className="col-6">
            <TabView>
              <TabPanel header="Application Details">
                <ApplicationDetails eid={eid} id={id} status={status} rowId={rowId} />
              </TabPanel>
              <TabPanel header="Survey No">
                <SurveyNo eid={eid} id={id} status={status} rowId={rowId} />
              </TabPanel>
              <TabPanel header="Taluk office Details">
                <TalukDetails eid={eid} id={id} status={status} rowId={rowId} />
              </TabPanel>
            </TabView>
          </div>
      ...
      >
        <form onSubmit={formik.handleSubmit}>
          <div className="form-group ">
            <label className="form-label">Upload Document:(pdf) <span style={{ color: "red" }}>*</span>  </label>
            <input
              type="file"
