...
function LayoutPendingPropertyEdit() {
  ...
  useEffect(() => {
    axios
      .get(`${API_BASE_URL}/layout/${id}`, {
        headers: {
          'Gl-Status':'pending', 
        },
      })
      .then((response) => {
        SetLand(response.data[0]);
  ...
  const [selectedSaleArea, setSelectedSaleArea] = useState("Select");
  ...
  const handleSaleArea = (event) => {
    setSelectedSaleArea(event.target.textContent);
  };
  ...
  const handleFileBrochure = (event) => {
  ...
  useEffect(() => {
    if (finalData) {
      const nameChange = finalData.map((item, index) => ({
        ...
        glk_plotno: item["Plot no"] ? item["Plot no"] : "",
        glk_sqft: item["sqft"] ? item["sqft"] : "",
        ...
        glk_roadwidth: item["Road width"] ? item["Road width"] : "",
        glk_landfacing: item["Plot facing"] ? item["Plot facing"] : "",
        glk_east_dimension: item["East Dimension"] ? item["East Dimension"] : "",
        glk_eastunit: item["East unit"] ? item["East unit"] : "",
        glk_west_dimension: item["West Dimension"] ? item["West Dimension"] : "",
        glk_westunit: item["West unit"] ? item["West unit"] : "",
        glk_south_dimension: item["South Dimension"] ? item["South Dimension"] : "",
        glk_southunit: item["South unit"] ? item["South unit"] : "",
        glk_north_dimension: item["North Dimension"] ? item["North Dimension"] : "",
        glk_northunit: item["North unit"] ? item["North unit"] : "", 
      }));
  ...
  useEffect(() => {
    setFormData({
      ...
      oldfile:
        selectedProperty != land.project_property ? land.project_document : [],
      currentfile:
        selectedProperty == land.project_property ? land.project_document : [],
      survey: nameChangeData.length > 0 ? nameChangeData : survey,
      areaunit:selectedValueArea?selectedValueArea:"",
      saleableunit:selectedSaleArea?selectedSaleArea:"",
  ...
  const handleSubmit = async (e) => {
    ...
    if (result.isValid) {
      ...
      form.append("saleable", formData.saleable);
      form.append("total_plot", formData.total_plot);
      form.append("available", formData.available);
      form.append("brochure", formData.brochure); 
      form.append("oldbrochure", formData.oldbrochure ? formData.oldbrochure : []); 
      form.append("areaunit", formData.areaunit); 
      form.append("saleableunit", formData.saleableunit); 
      form.append(
        "oldfile",
      ...
      try { 
        const response = await axios.post(`${API_BASE_URL}/layoutupdate`, form, {
          headers: {
            'Content-Type': 'multipart/form-data',
          },
        }); 
        if (response.status === 200) { 
          Toast({ message: "Layout Updated Successfully", type: "success" });
        } else { 
  ...
  useEffect(() => {
    if (land) {
      ...
      setSelectedValueArea(land.project_areaunit)
      setSelectedSaleArea(land.saleable_unit)
      setFormData({
        ...
        area:land.project_area,
        saleable:land.saleable_area,
        total_plot:land.total_plot,
        available:land.available_plot, 
        oldbrochure:land.brochure
      });
    }
                      ...
                      <div className="row">
                        ...
                        <div className="col-md-12 col-lg-4 mb-3 ">
                          ...
                          <div className="input-group">
                            <input type="text" className="form-control"  name="area" value={formData.area} onChange={handleChangeForm}/>
                            <button
                        ...
                        <div className="col-md-12 col-lg-4 mb-3 ">
                          ...
                          <div className="input-group">
                            <input type="text" className="form-control"  name="saleable" value={formData.saleable} onChange={handleChangeForm}/>
                            <button
                              className="btn btn-outline-secondary dropdown-toggle"
                              type="button"
                              data-bs-toggle="dropdown"
                        ...
                        <div className="col-md-12 col-lg-4 mb-3 ">
                          ...
                          <input
                            ...
                            className="form-control"
                            id="lastName"
                            name="total_plot" value={formData.total_plot} onChange={handleChangeForm}
                          />
                        </div>
                        ...
                        <div className="col-md-12 col-lg-4 mb-3 ">
                          ...
                          <input
                            ...
                            className="form-control"
                            id="lastName"
                            name="available" value={formData.available} onChange={handleChangeForm}
                          />
                        </div>
                        ...
                        <div className="col-md-12 col-lg-4 mb-3 ">
                          ...
                          <input
                            ...
                            className="form-control"
                            id="lastName"
                            onChange={handleFileBrochure}
                          />
                           {land.brochure ? 
                                //  <a href={`${IMG_PATH}/layout/${land.brochure}`} className="btn1 mt-1" download>
                                //     <FileDownloadIcon />
                                //   </a>
                                <button className="btn1" onClick={()=>viewFileUrl(`${IMG_PATH}/layout/${land.brochure}`)}>
                                <FileDownloadIcon />
                              </button>
                                  : null} 
                        </div>
...
export default LayoutPendingPropertyEdit;
