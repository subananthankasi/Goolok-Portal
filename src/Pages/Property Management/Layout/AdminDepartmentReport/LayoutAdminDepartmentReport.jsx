...
import AdminStatusUpdate from "./StatusModal";
import Spinner from 'react-bootstrap/Spinner';



function LayoutAdminDepartmentReport() {
  ...
  useEffect(() => {
    const fetchData = async () => {
        try {
            const response = await axios.get(`${API_BASE_URL}/admindpt`, {
                headers: { 
  ...
  let gridInstance;
    ...
    function gridUrlTemplate(props) { 
      return ( 
        <Link to={`/layout_admin_department?id=${props.id}&status=${statusUrl.status}`}
...
function completionPrecentage(props) { 
  ...
  return (
    <>
           <AdminStatusUpdate isOpen={isModalOpenStatus} closeModal={closeModalStatus} editData={editData} statusUrl={statusUrl.status}/>   
        ...
        <div className="container">
          ...
          <div className="col-12">
              <div className="card"> 
                ...
                <h4 className="page_heading">Layout Admin Department Report ({statusUrl.status})</h4>
...
export default LayoutAdminDepartmentReport;
