...
const PropertyCharges = ({basicDetails,surveyDetails,statusType}) => {
  ...
  const land = [
    ...
    },  
    {
      ...
      selector: (row) => row.admin_centprice || row["Goolok Rate per cent or any unit"],
      sortable: true,
      width:"150px",
      wrap: true,
    },  
    ...
    }, 
    {
      ...
      selector: (row) =>row.admin_total || row["Goolok Total cost"],
      sortable: true,
      wrap: true,
    }, 
  ...
  const handleSubmit = async(e) => {
   ...
   if(excelData.length == 0){
    ...
    const data = finalData.map((item)=>({ 
      id:item["id"], 
      admin_centprice:item["Goolok Rate per cent or any unit"] ? item["Goolok Rate per cent or any unit"] :null,
      admin_total:item["Goolok Total cost"] ? item["Goolok Total cost"] :null, 
    }))
  ...
  const formatData = {
    ...
    try{
         await axios.post(`${API_BASE_URL}/adminproperty`, formatData, {
        headers: {
