...
import { validateLandFormData } from "./validationLand";
...
function LandnewProperty() {
  ...
  const handlePlaceChanged = () => {
    ...
    const columns1 = [
      ...
      //   selector: (row) => row["Rate per cent or any unit"],
      //   sortable: true,
      //   width:"150px",
      //   wrap: true,
      // },
  ...
  useEffect(() => {
    axios.get(`${API_BASE_URL}/landtype`)
      .then(response => {
...
const [errors, setErrors] = useState({});
  ...
  const PropertyDocumentData = useSelector((state) => state.PropertyDocument.PropertyDocumentData); 
  const filteredData = PropertyDocumentData.filter(data => data.subproperty == selectedProperty); 
  useEffect(() => {
...
}, [posts,selectedState,selectedDistrict,selectedTaluk,selectedVillage,selectedPincode,selectedProperty,selectedFiles,nameChangeData,selectedVendor]);
  ...
  if (result.isValid) {
    ...
    if (selectedFiles.length > 0) {
     ...
     formData.survey.forEach((survey, index) => {
      ...
      try{
        await dispatch(addLandNewProperty(form)); 
      }catch (error){
...
const getVendor = selectedVendor?.value ? enableVendor.filter((data) => data.id === selectedVendor.value): [];
   return isLoaded ? (
    <>
                  ...
                  <form>
                    <div className={step === 1 ? "d-block" : "d-none"}>
                      <div className="row">
                        ...
                        <div className="mb-3 col-md-12 col-lg-4">
                          ...
                          <select id="inputState" className="form-select" onChange={handleChange} value={selectedProperty}>
                        ...
                        />
                          {errors.vendor && <div className="validation_msg">{errors.vendor}</div>} 
                        </div>
                      ...
                      {selectedVendor && (
                        ...
                        <div className="col-md-12 col-lg-4 mb-3 ">
                          ...
                          />
                          {errors.contactname && <div className="validation_msg">{errors.contactname}</div>}
                        </div>
                    ...
                    <div className={step === 3 ? "d-block" : "d-none"}>
                      <div className="row">
                        ...
                        <div className="text-end py-3 px-3">
                          <button className="btn1 me-1" onClick={(e) => { e.preventDefault(); firstPage();}}>
                            Previous
                          </button>
                          {/* <button className="btn1 me-2" onClick={(e) => {  handleSubmit(e) }}>Submit</button> */}
                          <button className="btn1 me-2" onClick={(e) => {  handleSubmit(e) }} disabled={isLoading}>
                          {isLoading ? (
                            <>
...
export default LandnewProperty;
