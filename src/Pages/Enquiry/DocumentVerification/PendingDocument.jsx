...
function PendingDocument() {
  ...
  const fetch = async (id) => {
    try {
      const response = await axios.get(`${API_BASE_URL}/enquiryreport?id=${id}&status=progress`, {
        headers: {
          "Gl-status": 'land'
        }
      })
  ...
  const handleRowSelect = (args) => {
    ...
    navigate(`/file_uploads/${rowData.id}/${rowData.userid}/pending`);
  };
