...
const TicketClosingApart = ({ eid, marketid, status, pagetype }) => {
    ...
    const customerColumns = [
        ...
        ...(staffid.logintype == "staff" && (status === "complete" || status === "pending")) && pagetype !== "reminder" && enquiryDoumentData?.status !== "booking" ? [
            // ? [
            {
                name: "Actions",
                cell: (row) => (
                    <div className="d-flex gap-2">
                        ...
                        <button
                            ...
                            data-tooltip-id="delete" onClick={() => handleEdit(row)}
                        >
                            <EditIcon />
                        </button>
                    </div>
    ...
    const formik = useFormik({
        ...
        validationSchema: yup.object().shape({
            ...
            facing_width: yup.string().required('facing_width is required !!').max(100),
            road_width: yup.string().required('road_width is required !!').max(100),
            corner_property: yup.string().required('corner_property is required !!').max(100),
            amenities: yup.string()
                .required('amenities is required !!')
                .matches(/^[A-Za-z\s]+$/, 'amenities must contain only letters and spaces'),
            facing: yup.string()
                .required('facing is required !!')
                .matches(/^[A-Za-z\s]+$/, 'facing must contain only letters and spaces'),
        }),
        onSubmit
    })
    ...
    const deleteUnitsDialogFooter = (
        <div className=" d-flex gap-3 justify-content-end">
            <button className="btn1" onClick={() => setDeleteDialog(false)}>No</button>
            <button className="btn1" onClick={handleDelete}>yes</button>
    ...
    return (
        <>
            ...
            <div className="card shadow border-0 mt-3">
                <div className="card shadow border-0 p-4">
                    <div className="d-flex justify-content-between">
                        ...
                        {staffid.logintype == "staff" && (status === "pending" || status === "complete") && pagetype !== "reminder" && customerdata.length === 0 ? (
                            <a
                                href="#0"
            ...
            <Dialog visible={editDialog} style={{ width: '55rem' }} breakpoints={{ '960px': '75vw', '641px': '90vw' }} header="Edit Ticket Closing" modal className="p-fluid" onHide={() => setEditDialog(false)} >
                <form onSubmit={formik.handleSubmit}>
                            ...
                            <div className="row">
                                ...
                                <div className="col-7 mb-3">
                                    ...
                                    {formik.errors.facing_width && formik.touched.facing_width ? (
                                        <p style={{ color: "red", fontSize: '12px' }}>{formik.errors.facing_width}</p>
                                    ) : null}
                                </div>
                            </div>
                        </div>
                            ...
                            <div className="row">
                                ...
                                <div className="col-7 mb-3">
                                    ...
                                    {formik.errors.amenities && formik.touched.amenities ? (
                                        <p style={{ color: "red", fontSize: '12px' }}>{formik.errors.amenities}</p>
                                    ) : null}
                                </div>
                            </div>
                        </div>
                            ...
                            <div className="row">
                                ...
                                <div className="col-7 mb-3">
                                    ...
                                    {formik.errors.corner_property && formik.touched.corner_property ? (
                                        <p style={{ color: "red", fontSize: '12px' }}>{formik.errors.corner_property}</p>
                                    ) : null}
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="d-flex justify-content-end mt-4">
                        <button className="btn1" type="submit" onClick={() => setEditing(true)}>Update</button>
                    </div>

                </form>
            </Dialog>

            <Dialog visible={deleteDialog} style={{ width: '32rem' }} breakpoints={{ '960px': '75vw', '641px': '90vw' }} header="Confirm" modal footer={deleteUnitsDialogFooter} onHide={() => setDeleteDialog(false)}>
...
export default TicketClosingApart