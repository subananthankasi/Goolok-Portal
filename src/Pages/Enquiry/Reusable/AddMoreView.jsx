...
export const AddMoreView = ({ isOpen, closeModal, id }) => {
  ...
  useEffect(() => {
    const fetchPatta = async () => {
      try {
        const response = await axios.get(`${API_BASE_URL}/enqaadhar/${id.id}`);
        ...
        <div className="modal-dialog modal-xl" role="document">
          <div className="modal-content">
            <div className="d-flex" style={{ alignItems: "center" }}>
              {id.doc_type == "Aadhaar" ? (
                <h4 className="page_subheading m-3"> {id.doc_type} Details</h4>
              ) : (
                <nav className="nav">
                  <a
