import { Autocomplete, GoogleMap, InfoWindow, Marker, Polygon, useJsApiLoader } from '@react-google-maps/api';
...
const ApartmentLocationMap = ({ eid, id, status, pagetype }) => {
  ...
  const handleConfirm = async () => {
    try {
      ...
      Toast({ message: "Successfully Updated", type: "success" });
      navigate("/apart_location_verification/4/4/complete")
    } catch (error) {
                ...
                {surveyData?.map((item, index) => (
                  <tr key={item.id}>
                    ...
                    {/* <td className="text-center">
                      ...
                      <button className="btn1 me-2" onClick={() => openDialog(item.id)} >Add</button>
                    </td> */}
                    <td className="text-center">
                      {item.location ? (
              ...
              >
                {
                  surveyData?.map((item, index) => {
                    ...
                    let polylinePath = [];
                    try {
                      polylinePath = item.location


                    } catch (error) {
                      console.error("Error parsing polyline path:", error);
                    }
                    return (
                      <>
                        <Marker
                        ...
                        />
                        <Polygon
                          path={[...polylinePath, polylinePath[0]]}
                          options={{
                            fillColor: "rgb(84, 209, 146)",
                            fillOpacity: 0.7,
                            strokeColor: "green",
                            strokeOpacity: 1,
                            strokeWeight: 2,
