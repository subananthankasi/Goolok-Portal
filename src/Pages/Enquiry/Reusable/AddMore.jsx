...
export const AddMore = ({ isOpen, closeModal, id }) => {
  ...
  const stateDropDown = useStateOptions(isOpen);
  const districtDropDown = useDistrictOptions();
  const talukDropDown = useTalukOptions(isOpen);
  const villageDropDown = useVillageOptions(isOpen);
  ...
  useEffect(() => {
    if (isOpen && previousDataPata) {
      ...
      const defaultOptionDistrict = districtDropDown.find(
        (option) => option.value === String(previousDataPata?.district) || ""
      );

      setSelectedDistrict(defaultOptionDistrict);

      const defaultOptionTaluk = talukDropDown.find(
        (option) => option.value === String(previousDataPata?.taluk) || ""
      );
  ...
  const handleAadharChange = (e) => {
    ...
    if (name === "aadhar_name" || name === "father_name") {
    ...
    // Aadhaar number validation + formatting
    if (name === "aadhar_number") {
      let rawNumber = newValue.replace(/\D/g, "");
      if (rawNumber.length > 12) {
        rawNumber = rawNumber.slice(0, 12);
      }
  ...
  const validateAadharForm = () => {
    let submitErrors = {};

    if (!aadharFormData.aadhar_name.trim()) {
      submitErrors.aadhar_name = "Aadhaar name is required";
    }

    if (!aadharFormData.father_name.trim()) {
      submitErrors.father_name = "Father name is required";
    }

    const digits = aadharFormData.aadhar_number.replace(/\D/g, "");
    if (!digits) {
      submitErrors.aadhar_number = "Aadhaar number is required";
    }

    if (!aadharFormData.address.trim()) {
  ...
  const handleAadharSubmit = async (e) => {
    e.preventDefault();
    const errors = validateAadharForm();
    setAadharError(errors);
  ...
  const [deedFormData, setDeedFormData] = useState({
    ...
    village: "",
    sro: "",
    areatype: "Rural_patta",
    ward: "",
    block: "",
  });

  const stateDeedDropDown = useStateOptions();
  const districtDeedDropDown = useDistrictOptions(isOpen);
  ...
  useEffect(() => {
    if (isOpen && id?.doc_type === "Title document") {
      fetchSro();
    }
  }, [isOpen, id?.doc_type]);
  ...
  const handleDeedTwoSubmit = async (e) => {
    ...
    const noError = ["units", "olddivision", "oldsurveyno"];
  ...
  useEffect(() => {
    if (id && id.doc_type == "Title document" && isOpen) {
      fetchDeed();
    }
  }, [id, isOpen]);
                            ...
                            (id.status === "pending" ||
                              ...
                              <div className="text-end">
                                <button
                                  ...
                                  onClick={() => {
                                    setAadharError({});
                                    setAadharFormData(emptyAadharForm);
                                  }}
                                >
                                  Clear
                                </button>
                          ...
                          (isLoading == 4 ? (
                            ...
                            <>
                              <div className="row mt-4">
                                ...
                                {deedFormData.areatype === "Town_patta" && (
                                  <>
                                    <div className="col-md-6 mb-3 ">
                                      <div className="row">
                                        <div className="col-4 mb-3 ">
                          ...
                          <>
                            <div className="row mt-3">
                              ...
                              >
                                ...
                                <Column
                                  field="survey_no"
                                  header={`${deedExistingData?.areatype === "Town_patta"
                                    ? "Town Survey No"
                                    : "Survey No"
                                    }`}
                                  className="p-2"
                                ...
                                {deedExistingData?.areatype ===
                                  "Town_patta" && (
                                    <Column
                                      field="old_survey"
                                      header="Old Survey No"
                                    ></Column>
                                  )}
                                {deedExistingData?.areatype ===
                                  "Town_patta" && (
                            ...
                            <div className="row mt-4">
                              ...
                              {deedExistingData?.areatype === "Town_patta" && (
                                <>
                                  <div className="col-md-6 mb-3 ">
                                    <div className="row">
                                      <div className="col-4 mb-3 ">
                                      ...
                                      <div className="col-8 mb-3 ">
                                        <input
                                          ...
                                          autoComplete="off"
                                          name="oldsurveyno"
                                          value={deedTwoFormData.oldsurveyno}
                                          // onChange={handleDeedTwoChange}
                                          onChange={(e) => {
                                            const onlyNumbers =
                                              e.target.value.replace(/\D/g, "");
                                    ...
                                    <div className="row">
                                      ...
                                      <div className="col-8 mb-3 ">
                                        <input
                                          ...
                                          autoComplete="off"
                                          name="olddivision"
                                          value={deedTwoFormData.olddivision}
                                          onChange={handleDeedTwoChange}
                                        />
                                      </div>
                                    </div>
