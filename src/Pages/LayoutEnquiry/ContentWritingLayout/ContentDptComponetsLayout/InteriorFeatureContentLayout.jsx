...
const InteriorFeatureContentLayout = ({ eid, id, status }) => {
    ...
    const ifeatureData = useSelector((state) => state.interiorFeatureData?.get?.data)
    ...
    return (
        <>
            ...
            <Dialog visible={newDialog} style={{ width: '32rem' }} breakpoints={{ '960px': '75vw', '641px': '90vw' }} header="Add Interior Features" modal className="p-fluid" onHide={hideDialog} >
                <form onSubmit={formik.handleSubmit} autoComplete="off">
                    <div>
                        <div className='form-group'>
                            <label htmlFor="interior" className='form-label'>Interior Features :</label>
                            <select
                                id='interior'
                                name='interior'
                            ...
                            >
                                <option value={''} > Select Features</option>
                                {
                                    ifeatureData?.map((item) => (
                                        <option key={item.id} value={item.id}> {item.feature_name} </option>
                                    ))
                                }

                            </select>
                            {formik.errors.interior && formik.touched.interior && (
                                <p style={{ color: "red", fontSize: '12px' }}>{formik.errors.interior}</p>
                            )}
                        </div>
            ...
            <Dialog visible={deleteDialog} style={{ width: '32rem' }} breakpoints={{ '960px': '75vw', '641px': '90vw' }} header="Confirm" modal footer={deleteUnitsDialogFooter} onHide={hideDeleteProductsDialog}>
                <div className="confirmation-content">
                    ...
                    <span style={{ marginLeft: '10px' }}>Are you sure you want to delete the Interior Features  ?</span>
                </div>
            </Dialog>
            ...
            <Dialog visible={editDialog} style={{ width: '32rem' }} breakpoints={{ '960px': '75vw', '641px': '90vw' }} header="Edit Interior Features" modal className="p-fluid" onHide={hideEditDialog} >
                <form onSubmit={formik.handleSubmit} autoComplete="off">
                    ...
                    <div className="d-flex justify-content-end gap-2 mt-4">
                        <button type='submit' className='btn1' onClick={() => setEditing(true)}>Update</button>
                    </div>
                </form>
            </Dialog>
        </>
...
export default InteriorFeatureContentLayout