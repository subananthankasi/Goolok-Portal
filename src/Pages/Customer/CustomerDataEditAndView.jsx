...
import PincodeDropDown, { usePincodeOptions } from "../../Utils/SelectDropDown/PincodeDropDown";
import StateDropDown, { useStateOptions } from "../../Utils/SelectDropDown/StateDropDown"; 
import axios from "axios";
import API_BASE_URL, { IMG_PATH } from "../../Api/api";
 

function CustomerDataEditAndView() {
  // for tab
  ...
  useEffect(() => {
    const fetchData = async () => { 
      try {
        if (id) {
          const response = await axios.get(`${API_BASE_URL}/customerData/${id}`);
...
//         (option) => option.value === formData.customer_state
//       );
//       const defaultOptionDistrict = districtDropDown.find(
//         (option) => option.value === formData.customer_district
//       );
//       const defaultOptionTaluk = talukDropDown.find(
//         (option) => option.value === formData.customer_taluk
//       );
//       const defaultOptionVillage = villageDropDown.find(
//         (option) => option.value === formData.customer_village
//       );
//       const defaultOptionPincode = pincodeDropDown.find(
//         (option) => option.value === formData.customer_pincode
//       ); 
...
//  },[])
  ...
  const handleChange = (e) => {
    ...
    const handleSubmit = (e) => {
      e.preventDefault()   
        dispatch(updateCustomerData(formData)) 
     };
                            ...
                            <div className="row">
                              ...
                              <div className="col-6">
                                <input
                                  ...
                                  onChange={handleChange}
                                />
                             {formData.aadhar_pic && (   <div className="mt-2">
                                  <a href={`${IMG_PATH}/${formData.aadhar_pic}`} className="btn1" target="_blank">
                                    View
                                  </a>
                                </div>)}
                              </div>
                        ...
                        <div className="col-md-12 col-lg-6">
                          ...
                          <div className="form-group mt-5">
                            <div className="row">
                             ...
                             {formData.aadhar_pic && ( <div className="mt-2">
                                  <a href={`${IMG_PATH}/${formData.passbook_pic}`} className="btn1" target="_blank">
                                    View
                                  </a>
                                </div>
                             )}
...
export default CustomerDataEditAndView;
