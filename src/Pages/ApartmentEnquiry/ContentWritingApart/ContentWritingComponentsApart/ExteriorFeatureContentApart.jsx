...
import { eEnqFeatureDeleteThunk, eEnqFeatureGetThunk, eEnqFeaturePostThunk, eEnqFeatureUpdateThunk } from '../../../../Redux/Actions/Enquiry/ContentWritingThunk/CWDescriptionFeatureThunk';
import Toast from '../../../../Utils/Toast';
import customStyle from '../../../../Utils/tableStyle';


const ExteriorFeatureContentApart = ({ eid, id, status }) => {
    ...
    return (
        <>
            ...
            <Dialog visible={newDialog} style={{ width: '32rem' }} breakpoints={{ '960px': '75vw', '641px': '90vw' }} header="Add Exterior Features" modal className="p-fluid" onHide={hideDialog} >
                <form onSubmit={formik.handleSubmit} autoComplete="off">
                    <div>
                        <div className='form-group'>
                            <label htmlFor="exterior" className='form-label'>Exterior Features :</label>
                            <select
                                id='exterior'
                                name='exterior'
                            ...
                            >
                                ...
                                {
                                    eData?.map((item) => (
                                        <option key={item.id} value={item.id}> {item.feature_exname} </option>
                                    ))
                                }

                            </select>
                            {formik.errors.exterior && formik.touched.exterior && (
                                <p style={{ color: "red", fontSize: '12px' }}>{formik.errors.exterior}</p>
                            )}
                        </div>
                        <div className='form-group mt-2'>
                            <label htmlFor="details" className='form-label'> Features Details:</label>
                            <input
                                id='details'
                                name='details'
                            ...
                            {formik.errors.details && formik.touched.details && (
                                <p style={{ color: "red", fontSize: '12px' }}>{formik.errors.details}</p>
                            )}
                        </div>
                    </div>
                    <div className="d-flex justify-content-end gap-2 mt-4">
                        <button type='submit' className='btn1' onClick={() => setEditing(false)}>Save</button>
                    </div>
                </form>
            </Dialog>
            {/*Delete modal */}
            ...
            <Dialog visible={deleteDialog} style={{ width: '32rem' }} breakpoints={{ '960px': '75vw', '641px': '90vw' }} header="Confirm" modal footer={deleteUnitsDialogFooter} onHide={hideDeleteProductsDialog}>
                <div className="confirmation-content">
                    ...
                    <span style={{ marginLeft: '10px' }}>Are you sure you want to delete the selected exterior Feature  ?</span>
                </div>
            </Dialog>

            {/*Edit modal */}

            <Dialog visible={editDialog} style={{ width: '32rem' }} breakpoints={{ '960px': '75vw', '641px': '90vw' }} header="Edit Exterior Features" modal className="p-fluid" onHide={hideEditDialog} >
                <form onSubmit={formik.handleSubmit} autoComplete="off">
                    <div>
                        <div className='form-group'>
                            <label htmlFor="exterior" className='form-label'>Exterior Features :</label>
                    ...
                    <div className="d-flex justify-content-end gap-2 mt-4">
                        <button type='submit' className='btn1' onClick={() => setEditing(true)}>Save</button>
                    </div>
                </form>
            </Dialog>
        </>
...
export default ExteriorFeatureContentApart