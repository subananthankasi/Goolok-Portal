...
const GeneralFeatureContentApart = ({ eid, id, status }) => {
    ...
    const gfeatureData = useSelector((state) => state.generalFeatureData?.get?.data)
    ...
    return (
        <>
            ...
            <Dialog visible={newDialog} style={{ width: '32rem' }} breakpoints={{ '960px': '75vw', '641px': '90vw' }} header="Add Genaral Features" modal className="p-fluid" onHide={hideDialog} >
                <form onSubmit={formik.handleSubmit} autoComplete="off">
                    <div>
                        <div className='form-group' >
                            <label htmlFor="features" className='form-label'>Genaral Features :</label>
                            <select
                                id='features'
                                name='features'
                            ...
                            >
                                <option value={''} > Select Features</option>
                                {
                                    gfeatureData?.map((item) => (
                                        <option key={item.id} value={item.id}> {item.feature_gname} </option>
                                    ))
                                }

                            </select>
                            {formik.errors.features && formik.touched.features && (
                                <p style={{ color: "red", fontSize: '12px' }}>{formik.errors.features}</p>
                            )}
                        </div>
                    ...
                    <div className="d-flex justify-content-end gap-2 mt-4">
                        <button className='btn1' type='submit' onClick={() => setEditing(false)}>Save</button>
                    </div>
                </form>
            </Dialog>
            {/*Delete modal */}
            ...
            <Dialog visible={deleteDialog} style={{ width: '32rem' }} breakpoints={{ '960px': '75vw', '641px': '90vw' }} header="Confirm" modal footer={deleteUnitsDialogFooter} onHide={hideDeleteProductsDialog}>
                <div className="confirmation-content">
                    ...
                    <span style={{ marginLeft: '10px' }}>Are you sure you want to delete selected Genaral Features  ?</span>
                </div>
            </Dialog>

            {/*Edit modal */}

            <Dialog visible={editDialog} style={{ width: '32rem' }} breakpoints={{ '960px': '75vw', '641px': '90vw' }} header="Edit Interior Features" modal className="p-fluid" onHide={hideEditDialog} >
                <form onSubmit={formik.handleSubmit} autoComplete="off">
                    <div>
                        <div className='form-group' >
                            <label htmlFor="features" className='form-label'>Genaral Features :</label>
...
export default GeneralFeatureContentApart