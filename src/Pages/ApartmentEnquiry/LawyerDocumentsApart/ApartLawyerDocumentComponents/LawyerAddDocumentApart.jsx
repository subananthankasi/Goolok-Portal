...
import { AddMoreLawerDocApart } from "./AddMoreLawerDocApart";
...
export const LawyerAddDocumentApart = (props) => {
    ...
    const fetchEnqData = async () => {
        try {
            const response = await axios.get(
                `${API_BASE_URL}/enquiryreport/${props.data.eid}`,
                {
                    headers: {
                        "Gl-Status": "lawyer",
                    },
    ...
    const anotherDocViewFetch = async (id) => {
        try {
            const response = await axios.get(
                `${API_BASE_URL}/${viewData?.lawyer_doc}/edit`
            );
    ...
    useEffect(() => {
        anotherDocViewFetch(props.data.id);
    }, []);
